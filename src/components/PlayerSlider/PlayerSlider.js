...
function PlayerSlider({ defaultSliderValue, sliderMinMax = [0, 1], value, stepValue = 1, name, onChange, option=null, label='', useSignificantFigures, ...props }) {
	...
	return (
		<div className="control-slider-container" {...props}>
			<Slider min={sliderMinMax[0]} max={sliderMinMax[1]} step={stepValue} value={sliderValue.toFixed(sigFigs)} onChange={(v) => changeValue(v)} />
            <div className="unit-input-container">
                <input min={sliderMinMax[0]} max={sliderMinMax[1]} type="text" value={sliderValue.toFixed(sigFigs)} onChange={(e) => changeValue(e.target.value)} onClick={handleClick} title={name} style={{width: `${sliderValue.toFixed(sigFigs).length}ch`}}/>
